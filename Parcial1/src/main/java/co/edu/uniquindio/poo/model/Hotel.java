...
public class Hotel {
    ...
    public String mostrarMatrizOcupacion(char[][] matriz){
        String mensaje = "Matriz de ocupacion (O = Ocupada, D = Disponible)\n";
        mensaje += "Orden de columnas: Lun  Mar  Mie  Jue  Vie  Sab  Dom\n\n";

        for(int i = 0; i < matriz.length; i++){
            mensaje += "Habitacion " + listaHabitaciones[i].getNumero() + ":   ";
            for(int j = 0; j < matriz[i].length; j++){
                mensaje += matriz[i][j] + "     ";
            }
            mensaje += "\n";
